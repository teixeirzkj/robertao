      ...
      <div className="mt-6 rounded-3xl border border-ink/10 bg-white p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="font-display text-2xl font-800 text-ink">{order.code}</p>
            <p className="mt-1 text-sm text-ink/55">{order.name}</p>
          </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === "aguardando" && (
          ...
          >
            ...
            <p className="flex items-center justify-center gap-2 text-center text-xs text-ink/35">
          ...
          >
                ...
                <div className="relative mt-4 space-y-2">
                  {order.prizes.map((prize, i) => (
                    <motion.div
                      ...
                      className="flex items-center gap-3 rounded-2xl bg-paper/95 p-3 text-left text-ink"
                    >
                      {prize.image && (
                        <img
                          src={prize.image}
            ...
            <div className="rounded-3xl border border-ink/10 bg-white p-5 sm:p-6">
                    ...
                    <motion.span
                      ...
                      className={
                        isPrize
                          ...
                          : "rounded-lg border border-ink/10 bg-paper-100 px-2.5 py-1.5 font-mono text-xs text-ink/80"
                      }
                    >
              ...
              <p className="mt-4 text-[11px] leading-relaxed text-ink/35">
                Guarde o código <strong className="text-ink/65">{order.code}</strong> — você
        ...
        {phase === "cancelado" && (
          ...
          >
            ...
            <p className="mt-3 font-display text-lg font-700 text-ink">Pedido cancelado</p>
            <p className="mt-1 text-sm text-ink/55">
  ...
  return (
    <div className="flex gap-1.5">
      {values.map((v, i) => (
        <span
          key={i}
          className="rounded-lg border border-gold/30 bg-paper px-3 py-2 font-mono text-sm text-gold/85"
        >
          {padTicket(v, total)}
        </span>
      ))}
